...
SELECT count("ДЕНЬ") AS "ЧИСЛО РОЖДЕНИЯ БЕЗ УЧЕТА ПОВТОРЕНИЙ"
FROM BirthDayTable;


--ex4
WITH RemoteLearningForm AS (
    SELECT
...
--ex5
WITH AverageAgeGroup1101 AS (
    SELECT avg(date_part('year', NOW()) - date_part('year', "Н_ЛЮДИ"."ДАТА_РОЖДЕНИЯ")) AS AverageAge
    FROM "Н_УЧЕНИКИ"
             JOIN "Н_ЛЮДИ" ON "Н_ЛЮДИ"."ИД" = "Н_УЧЕНИКИ"."ЧЛВК_ИД"
    WHERE "Н_УЧЕНИКИ"."ГРУППА" = '1101'
)
SELECT "Н_УЧЕНИКИ"."ГРУППА",
       avg(date_part('year', NOW()) - date_part('year', "Н_ЛЮДИ"."ДАТА_РОЖДЕНИЯ")) AS СРЕДНИЙ_ВОЗРАСТ
FROM "Н_УЧЕНИКИ"
         JOIN "Н_ЛЮДИ" ON "Н_ЛЮДИ"."ИД" = "Н_УЧЕНИКИ"."ЧЛВК_ИД"
GROUP BY "Н_УЧЕНИКИ"."ГРУППА"
HAVING avg(date_part('year', NOW()) - date_part('year', "Н_ЛЮДИ"."ДАТА_РОЖДЕНИЯ")) < (SELECT AverageAge FROM AverageAgeGroup1101);
